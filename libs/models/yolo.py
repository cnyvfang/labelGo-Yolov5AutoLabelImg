...
from libs.models.common import NMS
...
from libs.detect_utils.torch_utils import time_synchronized, fuse_conv_and_bn, model_info, scale_img, initialize_weights, \
...
class ASFF_Detect(nn.Module):   #add ASFFV5 layer and Rfb
    ...
    def __init__(self, nc=80, anchors=(), multiplier=0.5,rfb=True,ch=()):  # detection layer
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        # elif m is Concat: #自修改 BiFPN
        #     c2 = max([ch[x] for x in f])
        elif m is Detect:
            args.append([ch[x] for x in f])
            if isinstance(args[1], int):  # number of anchors
                args[1] = [list(range(args[1] * 2))] * len(f)
        elif m is ASFF_Detect:  #修改部分 ASFF Detect
            args.append([ch[x] for x in f])
            if isinstance(args[1], int):  # number of anchors
                args[1] = [list(range(args[1] * 2))] * len(f)
        elif m is Contract:
            ...
            c2 = ch[f] // args[0] ** 2
        elif m is SELayer:  # 修改部分 SELayer
